...
$character = new Character('Cammy', UP . UP . DOWN . DOWN);

addStandardMovesets($character);

$character->addMoveset(
  'Spiral Arrow (Cannon Drill)', SPECIAL, movesetImages("./special-1/*.jpg"), "16, 18, 8+19", STANDING | CROUCHING, KNOCKDOWN,
  array(QCF, KICK), array(array(8,6,SKIP,15,18),
array(8,6,SKIP,22,26),
array(8,6,SKIP,22,28))
);

$character->addMoveset(
  'Cannon Spike', SPECIAL, movesetImages("./special-2/*.jpg"), "20, 20, 20", STANDING | CROUCHING, KNOCKDOWN,
...
$character->addMoveset(
  'Spinning Knuckle', SPECIAL, movesetImages("./special-3/*.jpg"), "8+17, 9+18,10+19", STANDING | CROUCHING, 'Pushback + Knockdown',
...
$character->movesets["close-mp"]->frames  = array(3,2,4,SKIP,5);     
$character->movesets["close-hp"]->frames  = array(4,5,7);       
